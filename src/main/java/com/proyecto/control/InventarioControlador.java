...
public class InventarioControlador extends AbstractControlador<Inventario> {
    ...
    public int siguienteNumero(Clase clase) {
        String jpql = "SELECT COUNT(a) FROM Inventario a WHERE a.bien.clase = :clase";
        List<Object[]> lista = this.getDao().getEntityManager().createQuery(jpql).setParameter("clase", clase).getResultList();
        int conteo = ((Number) lista.get(0)[0]).intValue();
        return conteo + 1;
    }
